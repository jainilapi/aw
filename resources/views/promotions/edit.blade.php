...
@section('content')
<form id="promotionForm" method="POST" action="{{ route('promotions.update', encrypt($promotion->id)) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row g-4">
        <!-- Main Form Column -->
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <input type="hidden" name="type" id="promotionType" value="{{ old('type', $promotion->type) }}" required>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $promotion->name) }}" placeholder="e.g., Summer Sale 2026" required>
                            @error('name')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Promo Code <span class="text-danger">*</span></label>
                            <input type="text" name="code" class="form-control text-uppercase" value="{{ old('code', $promotion->code) }}" placeholder="e.g., SUMMER20" required>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="discount_type" class="form-select discount-type-select type-field">
                                    <option value="0" {{ old('discount_type', $promotion->discount_type) == false ? 'selected' : '' }}>Percentage (%)</option>
                                    <option value="1" {{ old('discount_type', $promotion->discount_type) == true ? 'selected' : '' }}>Fixed Amount (₹)</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Discount Value <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control type-field" value="{{ old('discount_amount', $promotion->discount_amount) }}" step="0.01" min="0" placeholder="Enter value">
                                    <span class="input-group-text discount-symbol">{{ $promotion->discount_type ? '₹' : '%' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="product_id[]" id="productSelect" class="form-select" multiple data-placeholder="Search and select products...">
                                @foreach($selectedProducts as $prod)
                                    <option value="{{ $prod->id }}" selected>{{ $prod->name }} [{{ ucfirst($prod->type) }}]</option>
                                @endforeach
                            </select>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Discount Value <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control prodisc-discount-amount type-field" value="{{ old('discount_amount', $promotion->discount_amount) }}" step="0.01" min="0" placeholder="Enter value">
                                    <span class="input-group-text discount-symbol prodisc-discount-symbol">{{ $promotion->discount_type ? '₹' : '%' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="cart_minimum_amount" class="form-control" value="{{ old('cart_minimum_amount', $promotion->cart_minimum_amount) }}" step="0.01" min="0" placeholder="Enter minimum cart total">
                                </div>
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Discount Value <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control cardisc-discount-amount type-field" value="{{ old('discount_amount', $promotion->discount_amount) }}" step="0.01" min="0" placeholder="Enter value">
                                    <span class="input-group-text discount-symbol cardisc-discount-symbol">{{ $promotion->discount_type ? '₹' : '%' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="x_product" id="xProductSelect" class="form-select" data-placeholder="Search product...">
                                        ...
                                        @if($promotion->xProduct)
                                            <option value="{{ $promotion->xProduct->id }}" selected>{{ $promotion->xProduct->name }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6" id="xVariantContainer" style="{{ $promotion->x_variant ? '' : 'display: none;' }}">
                                    <label class="form-label">Select Variant</label>
                                    <select name="x_variant" id="xVariantSelect" class="form-select">
                                        <option value="">Select Variant</option>
                                        @if($promotion->xVariant)
                                            <option value="{{ $promotion->xVariant->id }}" selected>{{ $promotion->xVariant->name ?? 'Variant #' . $promotion->xVariant->id }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6" id="xUnitContainer" style="{{ $promotion->x_unit ? '' : 'display: none;' }}">
                                    ...
                                    <select name="x_unit" id="xUnitSelect" class="form-select">
                                        ...
                                        @if($promotion->x_unit)
                                            <option value="{{ $promotion->x_unit }}" selected>Unit #{{ $promotion->x_unit }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Quantity to Buy <span class="text-danger">*</span></label>
                                    <input type="number" name="x_quantity" class="form-control" value="{{ old('x_quantity', $promotion->x_quantity ?? 1) }}" min="1" placeholder="e.g., 2">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="y_item" id="yProductSelect" class="form-select" data-placeholder="Search product...">
                                        <option value="">Select Product</option>
                                        @if($promotion->yProduct)
                                            <option value="{{ $promotion->yProduct->id }}" selected>{{ $promotion->yProduct->name }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6" id="yVariantContainer" style="{{ $promotion->y_variant ? '' : 'display: none;' }}">
                                    <label class="form-label">Select Variant</label>
                                    <select name="y_variant" id="yVariantSelect" class="form-select">
                                        <option value="">Select Variant</option>
                                        @if($promotion->yVariant)
                                            <option value="{{ $promotion->yVariant->id }}" selected>{{ $promotion->yVariant->name ?? 'Variant #' . $promotion->yVariant->id }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6" id="yUnitContainer" style="{{ $promotion->y_unit ? '' : 'display: none;' }}">
                                    <label class="form-label">Select Unit <span class="text-danger">*</span></label>
                                    <select name="y_unit" id="yUnitSelect" class="form-select">
                                        <option value="">Select Unit</option>
                                        @if($promotion->y_unit)
                                            <option value="{{ $promotion->y_unit }}" selected>Unit #{{ $promotion->y_unit }}</option>
                                        @endif
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Quantity to Get <span class="text-danger">*</span></label>
                                    <input type="number" name="y_quantity" class="form-control" value="{{ old('y_quantity', $promotion->y_quantity ?? 1) }}" min="1" placeholder="e.g., 1">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <div id="descriptionEditor" class="form-control" style="min-height: 150px;">{!! old('description', $promotion->description) !!}</div>
                        <input type="hidden" name="description" id="descriptionInput">
                        @error('description')<div class="text-danger small">{{ $message }}</div>@enderror
                    </div>
                    <div class="mb-4">
                        <label class="form-label">How to Use <span class="text-danger">*</span></label>
                        <div id="howToUseEditor" class="form-control" style="min-height: 150px;">{!! old('how_to_use', $promotion->how_to_use) !!}</div>
                    ...
                    <div class="mb-0">
                        ...
                        <div id="termsEditor" class="form-control" style="min-height: 150px;">{!! old('terms_and_condition', $promotion->terms_and_condition) !!}</div>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="start_date" id="startDate" class="form-control" value="{{ old('start_date', $promotion->start_date ? $promotion->start_date->format('Y-m-d H:i') : '') }}" placeholder="Select start date & time" required readonly>
                            @error('start_date')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">End Date & Time <span class="text-danger">*</span></label>
                            <input type="text" name="end_date" id="endDate" class="form-control" value="{{ old('end_date', $promotion->end_date ? $promotion->end_date->format('Y-m-d H:i') : '') }}" placeholder="Select end date & time" required readonly>
                            @error('end_date')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Application Limit <span class="text-danger">*</span></label>
                            <input type="number" name="application_limit" class="form-control" value="{{ old('application_limit', $promotion->application_limit ?? 1) }}" min="1" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-check form-switch mt-2">
                                <input class="form-check-input" type="checkbox" id="autoApplicable" name="auto_applicable" value="1" {{ old('auto_applicable', $promotion->auto_applicable) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-check form-switch mt-2">
                                <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ old('status', $promotion->status) ? 'checked' : '' }}>
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-4 sticky-top" style="top: 20px;">
                ...
                <div class="card-body">
                    <div class="poster-upload-zone {{ $promotion->poster_url ? 'has-image' : '' }}" id="posterUploadZone">
                        <input type="file" name="poster" id="posterInput" class="d-none" accept="image/jpeg,image/png,image/webp">
                        <div id="posterPlaceholder" class="{{ $promotion->poster_url ? 'd-none' : '' }}">
                        ...
                        <img id="posterPreview" src="{{ $promotion->poster_url ?? '' }}" class="poster-preview {{ $promotion->poster_url ? '' : 'd-none' }}" alt="Poster Preview">
                    </div>
                    @error('poster')<div class="text-danger small mt-2">{{ $message }}</div>@enderror
                    <div class="mt-3">
                        <button type="button" class="btn btn-outline-secondary btn-sm w-100" id="changePoster" style="{{ $promotion->poster_url ? '' : 'display: none;' }}">
...
$(document).ready(function() {
    ...
    // Initialize DateTimePickers with existing values
    const startDateVal = '{{ old('start_date', $promotion->start_date ? $promotion->start_date->format('Y-m-d H:i') : '') }}';
    const endDateVal = '{{ old('end_date', $promotion->end_date ? $promotion->end_date->format('Y-m-d H:i') : '') }}';
    ...
    $('#startDate').daterangepicker({
        ...
        autoApply: true,
        startDate: startDateVal ? moment(startDateVal, 'YYYY-MM-DD HH:mm') : moment(),
        locale: {
    ...
    $('#endDate').daterangepicker({
        ...
        timePickerSeconds: false,
        autoApply: true,
        startDate: endDateVal ? moment(endDateVal, 'YYYY-MM-DD HH:mm') : moment(),
        locale: {
            format: 'YYYY-MM-DD HH:mm'
        }
    });
    ...
    $('.type-card[data-type="{{ old('type', $promotion->type) }}"]').click();
